...
findInScope
  ...
  iden =
   ...
   where
    createError m _ = AnalysisError IdentifierAlreadyDefinedError m Nothing
     ...
     in
      case a of
        ...
        Right a' -> case (findInScope (currentScope a') varName) of
          ...
          Nothing ->
            let
              updatedScope = (currentScope a'){variables = Map.insert varName (VI{viName = varName, viType = varType}) (variables (currentScope a'))}
             in
     ...
     in
      case (analyzeFormalParamList updatedNewScope (fParams fn)) of -- 3. Verify function parameters have unique names
        ...
        Right analyzedNewScope ->
          let
            updatedParentScope = currScope{functions = Map.insert fnName (FI{fiName = fnName, fiParams = convertToFormalParam (fParams fn), fiResType = fnRetType}) (functions currScope)}
    ...
    let
     ...
     in
      case (analyzeFormalParamList newScope (pParams pr)) of -- 2. Verify procedure parameters have unique names
        ...
        Right analyzedNewScope ->
          let
            updatedParentScope = currScope{procedures = Map.insert prName (PRI{priName = prName, priParams = convertToFormalParam (pParams pr)}) (procedures currScope)}
            finalNewScope = analyzedNewScope{parentScope = Just updatedParentScope}
            updatedAnalyzer = Right a{globalScope = if (isGlobalScope updatedParentScope) then updatedParentScope else (globalScope a), currentScope = finalNewScope}
           in
...
convertToFormalParam :: [FormalParam] -> [ParamInfo]
convertToFormalParam = map (\p -> PAI{paiName = idValue (fpName p), paiType = fpType p})
